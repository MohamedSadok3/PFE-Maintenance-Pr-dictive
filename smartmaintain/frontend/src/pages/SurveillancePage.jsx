...
function SurveillancePage() {
  ...
  const alertHeatmapData = Array.from({ length: 24 }, (_, hour) => {
    const hourDefects = defectHistory.filter(d => {
      const defectHour = new Date(d.timestamp).getHours()
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {defectHistory.slice(0, 10).map((item, idx) => (
                      ...
                      >
                        <td className="py-3 px-2 text-slate-700">{formatTime(item.timestamp)}</td>
              ...
              {lastDefect ? (
                <div className="space-y-3">
                  ...
                  <div className="pt-2 border-t border-slate-100">
                    <p className="text-xs text-slate-400">{formatTime(lastDefect.timestamp)}</p>
                  </div>
                </div>
              ) : (
                <EmptyState
